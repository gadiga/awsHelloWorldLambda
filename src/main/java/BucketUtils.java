...
public class BucketUtils {
	...
	public static void listAllBuckets(AmazonS3Client s3Client) throws Exception {
		s3Client.listBuckets().stream().forEach(bucket -> {
			System.out.println("showing bucket.." + bucket.getName());
			try {
				BucketUtils.showBucket(bucket.getName(), s3Client);
			} catch (IOException e) {
	...
	public static void showBucket(String bucketName, AmazonS3Client s3client) throws IOException {

		try {
			System.out.println("Listing S3 bucket: " + bucketName);
			ObjectListing objectListing = s3client.listObjects(bucketName);

			while (true) {
				for (Iterator<?> iterator = objectListing.getObjectSummaries().iterator(); iterator.hasNext();) {
					S3ObjectSummary objectSummary = (S3ObjectSummary) iterator.next();
					System.out.println(bucketName + "~~" + s3client.getBucketLocation(bucketName));
				}
			...
			for (Iterator<?> iterator = list.getVersionSummaries().iterator(); iterator.hasNext();) {
				S3VersionSummary s = (S3VersionSummary) iterator.next();
				System.out.printf("%1$s :: %2$s :: %3$s", bucketName, s.getKey(), s.getVersionId());
			}
